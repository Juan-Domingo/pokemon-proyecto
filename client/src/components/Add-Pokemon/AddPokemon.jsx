...
function AddPokemon({ history }) {
        ...
        const exists = async () => {
            try {
                ...
                if (input.name.length !== 0 && res) {
                    setErrorsName('Este nombre ya existe!');
                }
            } catch (err) {
                console.error(err);
            }
    ...
    const handleChange = (e) => {
        ...
        if (!input.name) {
            setErrorsName('Nombre es requerido.');
        } else if (!regexName.test(input.name)) {
            setErrorsName('Nombre es invalido.');
        } else {
            setErrorsName(null);
        }

        if (!input.attack) {
            setErrorsNumbers('Este atributo esta perdido.');
        } else if (!Number.isInteger(Number(input.attack))) {
            setErrorsNumbers('Debe ser un numero.');
        } else {
        ...
        <form onSubmit={handleSubmit} className={styles.formContainer}>
            <section className={styles.inputs}>
                ...
                <input
                    ...
                    placeholder='Height…'
                />
                <label htmlFor='spriteSrc'>Imagen</label>
                <input
                    type='text'
                    name='spriteSrc'
                    id='link'
